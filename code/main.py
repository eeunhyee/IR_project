...
# ES 설정
es_username = "elastic"
es_password = "Lf0rt7tgbgMhKIUYYSC6"
ES_INDEX = "test"
...
# 1) Chunking
# =========================
def chunk_text(text: str, chunk_size: int = 100, chunk_overlap: int = 150) -> List[str]:
    """
    간단한 문자 기반 청킹.
...
def make_chunk_docs(docs: List[Dict[str, Any]], chunk_size: int = 100, chunk_overlap: int = 150) -> List[Dict[str, Any]]:
    """
...
# =========================
def hybrid_search_with_rerank(query: str, k_final: int = 3, bm25_k: int = 50, knn_k: int = 50) -> Tuple[List[str], List[Dict[str, Any]]]:
...
def answer_question(messages: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    if result.choices[0].message.tool_calls:
        ...
        # ✅ Hybrid + Rerank 검색
        topk_docids, references = hybrid_search_with_rerank(standalone_query, k_final=3, bm25_k=50, knn_k=50)
...
if __name__ == "__main__":
    ...
    # 3) ✅ 청킹
    chunked_docs = make_chunk_docs(raw_docs, chunk_size=100, chunk_overlap=150)
    print(f"raw docs: {len(raw_docs)} -> chunked docs: {len(chunked_docs)}")

    # 4) ✅ chunk 단위 임베딩 생성
    embeddings = get_embeddings_in_batches(chunked_docs, batch_size=8)
